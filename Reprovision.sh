...
setup_project() {
    ...
    if [ ! -f .env ]; then
        # Buscar .env.example en el proyecto (recursivamente si es necesario)
        local env_file=$(find . -name ".env.example" -type f 2>/dev/null | head -n 1)
        if [ -n "$env_file" ]; then
            cp "$env_file" .env
        else
            warn "No se encontró .env.example"
        fi
    fi
